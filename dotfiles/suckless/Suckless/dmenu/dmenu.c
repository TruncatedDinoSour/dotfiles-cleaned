...
static char *embed;
static int bh, mw, mh;
static unsigned int inputw = 0, promptw;
...
static void setup(void) {
    ...
    if (parentwin == root && (info = XineramaQueryScreens(dpy, &n))) {
        ...
        mw = MIN(MAX(max_textw() + promptw, 100), info[i].width) + dmenuwpx;
...
#endif
    {
        ...
        mw = MIN(MAX(max_textw() + promptw, 100), (unsigned int)wa.width) +
             dmenuwpx;
        x = (wa.width - mw) / 2;
        y = (wa.height - mh) / 2;
    }
    ...
    for (item = items; item && item->text; ++item) {
        if ((tmp = textw_clamp(item->text, mw / 3)) > inputw) {
            if ((inputw = tmp) == (unsigned int)mw / 3)
                break;
        }
    }
